...
- 改善 KKTIX 登入的人機驗證判定與帳密錯誤回報，不再反覆重試
- 修復登入頁內嵌人機驗證未通過就被當成成功（KKTIX、cityline）
- 修復 TixCraft 剩餘座位不足設定張數仍選入該區 (#358)
- 修復排除關鍵字在改選第一個可選項時失效，仍可能選到輪椅、身障席
- 新增 TicketPlus 卡友專屬活動自動填入信用卡前六碼
- 修復 TicketPlus 下一步點不到與隱藏提示視窗誤判為排隊 (#389)
- 封鎖 iBon 正式站會員追蹤程式與多站廣告追蹤
- 設定頁固定分頁列與操作列，儲存按鈕改用高對比配色
